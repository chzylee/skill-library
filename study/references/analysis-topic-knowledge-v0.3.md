...
**Class:** informational search + light analysis. **Status:** v0.3, EXPERIMENTAL — do not use
outside the comparison run.
...
| Stage 3A — Verify | the verify agent |
| Stage 3B — Absence | the absence agent |
...
One JSON object per line. Field names exactly as below. 15 fields.
...
```json
{
  ...
  "evidence": "re-opened",
  "quote": "verbatim sentence from the source that supports this claim, <=300 chars",
  "depth": "operation",
...
**`quote` (new in v0.3).** Required whenever `evidence` is `re-opened`. Empty string when
...
**For every row citing a source, copy the verbatim sentence that supports the claim into `quote`.
Do not paraphrase it. If you cannot find a single sentence that supports the claim, that is a
signal the claim is your inference rather than the source's — either fix the claim or mark
`evidence: authored`.**
...
One agent, independent of the harvest. Mechanical: does this row's `quote` support its
`description`; is this URL live.

**Read the rows and check each `quote` supports its `description`.** Re-fetch only: every
`exercise` row, every row containing a number, every URL that looks templated or unpinned, plus a
random 15% of the remainder. **You must log exactly which rows you re-fetched and which you took
on the quote alone. No silent sampling.**

A row whose `quote` does not support its `description`, or whose re-fetched source does not
support its claim, is `killed` with the reason recorded — not quietly fixed. Correct `evidence`
where it was overstated. **Report coverage as a fraction and name what you did not check.**

Also check: prohibited fields; whether the `quote` field is populated wherever `evidence` is
`re-opened` and empty wherever `evidence` is `authored`; whether the drills actually exercise what
they claim.

Append your verdicts to `audit-verdicts.jsonl` as one record per changed or added row; the
orchestrator applies them.

## Stage 3B — Absence

Unchanged from v0.2's brief 1. One agent, independent of the harvest, on the strongest model
available.

**Look for absence, not only for error.** Write your own list of the three to five things anyone
studying this topic obviously must know **before reading the rows**, then check it against them.
Confirm each is present (cite the row ID) or genuinely outside the declared boundary. Anything
missing is a DISCOVERY finding — write the row yourself in full schema. In v0.1 this brief
produced the run's best finding: every "additive changes are safe" claim was silently conditional
on a principle that was never harvested, from a file the harvest had opened twice. A pipeline
briefed only to verify what is present cannot notice what is missing.

Also check: whether `judgment` is being used as a general "advanced" bucket; `depth` assignments
inconsistent with the table or with their own `depth_check` sentence.
