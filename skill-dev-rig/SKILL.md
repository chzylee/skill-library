...
form, and dev/prod build separation. **The three are independent.** Rig one,
two, or all three, now or later. Running this again to add a part you skipped
...
- **All three no** → report that nothing needed rigging, and stop. This is a
...
End by reporting: which parts were rigged and which were skipped, every file
written, the ender set chosen and why, the marker name, and — if the repo was
newly rigged for dev/prod — the note that cloners get one approval prompt for
