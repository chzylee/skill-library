#!/usr/bin/env node
// config-form — a generic, schema-driven configuration form, running ON the
// local-ui engine (this file is the engine's first real consumer).
//
// Domain-agnostic ON PURPOSE: it knows nothing about any particular skill. It
// renders a form from a schema, collects values + which tasks the user asked to
// run, writes the values to a config file, and hands the requested task ids
// back to the caller. Anything that needs credentials or tools (creating pages,
// registering cron) is NOT done here — the calling skill does that.
//
// Node stdlib only — vendorable alongside local-ui.mjs. Reshaped from the
// standalone config-form.mjs v0.1.0; the server half now lives in the engine.
// Dropped from the standalone version, deliberately:
//   --lock       (kill-by-PID-file kills unrelated processes under PID reuse;
//                 if you launch twice, the stale tab's save simply fails)
//   --url-file   (superseded by the engine's CONFIG_FORM_RESULT= run record)
//
// Usage:
//   node config-form.mjs --schema <path> [--values <path>] [--status <path>]
//                        --out <path> [--tasks-out <path>] [--result <path>]
//                        [--title <str>] [--port 0] [--timeout <sec>] [--open]
//
// Contract (stdout markers the caller parses — the engine prints them):
//   CONFIG_FORM_URL=<url>       listening; browser opened if --open
//   CONFIG_FORM_RESULT=<path>   JSON run record (reason/exitCode/summary)
//   CONFIG_FORM_SUMMARY … CONFIG_FORM_DONE   fence after save; the legacy
//                               `CONFIG_FORM_SAVED out=<p> tasks=<p>` line is
//                               kept inside the fence for existing parsers
//   CONFIG_FORM_TIMEOUT         no submit before --timeout (default 540); exit 3
//   CONFIG_FORM_ERROR <msg>     precondition failed; exit 2
//
...
function renderGroup(g) {
  ...
  // create-it-for-me stay together). A task that provides a config key with no field still
  // renders — at the group bottom.
...
function save(e){e.preventDefault();fetch('/api/save?token=__TOKEN__',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(collect())}).then(function(r){if(!r.ok)throw 0;return r.json();}).then(function(){document.getElementById('done').classList.add('show');}).catch(function(){alert('Save failed — is the terminal process still running?');});return false;}
...
await serveUI({
  ...
  routes: {
    'POST /api/save': (body, ctx) => {
      writeConfig(body);
      saved = true;
      ctx.end('done'); // the save IS the ender — no second fetch for a closing tab to lose
      return { ok: true };
    },
  },
  // The oneshot preset: submit ends it, tab-close doesn't (an abandoned form
  // should sit until the clock, so a re-found tab still works), clock backstops.
  enders: {
    done: true,
    close: false,
