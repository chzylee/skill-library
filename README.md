...
| **[landscape-search](landscape-search/README.md)** | Finds what the world already thinks about a problem space, then reasons about where the conventional answer is wrong *for your case*. A sub-agent does the searching; nothing leaves your machine until you approve the exact query strings. | Anyone about to build on a default approach who wants to know whether the default is actually right here. | ✅ Fully (better with an Exa MCP connector; falls back to WebSearch) |
